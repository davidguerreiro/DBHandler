...
class Db{
	...
	function __construct(){
		...
		$mysql_server = 'localhost';										//Server where we want to establish a connection
		$mysql_login = 'root';												//MySql user with privileges
		//$password = '';													//MySql pass if required
		$database_name = 'databasename';									//Database name

		$this->rows = 0;													//Number of rows affected

		$this->connection = ($password != '') ? mysqli_connect($mysql_server, $mysql_login, $password) : mysqli_connect($mysql_server, $mysql_login);		//Establishing database connection. It is different depending wheter we use a password or not

		settype($this->conenction, 'boolean');
		mysqli_select_db($this->connection, $database_name);

		$this->database_selected = $database_name;							//Database that we are working with
		$this->last_id = 0;													//id generated in the last query
	...
	*/
	...
	/**
	*
	* This method returns the id generated by the last query
	*
	* @return int $this->last_id id generated by the last query
	*
	*/
	...
	*/
	public function query($query){
		...
		$last_id = ($status) ? mysqli_insert_id() : false;
		$this->last_id = (!$last_id) ? $this->last_id : $last_id;
